...
## 録音音声をGoogleドライブに自動保存（任意）
設定タブの **「Googleドライブに自動保存」** で、録音するたびに自分のGoogleドライブへ自動アップロードできる。
- サーバー不要のクライアントサイドOAuth。スコープは **`drive.file`**（アプリが作ったファイルのみ／ドライブ全体は読まない）
- 保存先：`（フォルダ名）/（受験者_日付）/A-1_見出し.webm`
- **設定手順は `SETUP-GOOGLE-DRIVE.md`**（Google CloudでOAuthクライアントID作成 → アプリに貼付 → 接続 → 自動保存ON）
- 用途：どの端末で録音しても音声が社長のドライブ1か所に集約 → PCでまとめて文字起こし
- クライアントIDは公開識別子なのでlocalStorage保存。アクセストークンはメモリのみ（約1時間で失効、再接続で更新）
